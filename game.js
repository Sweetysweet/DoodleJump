window.requestAnimFrame = (function() {
  ...
  // спрайт для платформы
    ...
    //1: нормальная
    //2: движущаяся
    //3: Ломающаяся платформа
    //4: исчезающая платформа 
    ...
    // мы не можем иметь 2 подряд платформы которые будут ломаться
  ...
  function init() {
    ...
    // функция которая будет очищать поле при каждом кадре
      ...
      function playerCalc() {
        ...
        //ускорение при зажимании клавиш
        ...
        //лимит скорости при ускорении
        if(player.vx > 8)
          player.vx = 8;
        else if(player.vx < -8)
          player.vx = -8;

        // прыжок игрока когда он находится на платформе
        if ((player.y + player.height) > base.y && base.y < height) player.jump();

        //конец игры если игрок падает под платформы
        if (base.y > height && (player.y + player.height) > height && player.isDead != "lol") player.isDead = true;

        //движение игрока через экран

        if (player.x > width) player.x = 0 - player.width;
        else if (player.x < 0 - player.width) player.x = width;

        // добавление эффекта гравитации
        ...
        // Когда игрок достигнет половины высоты, переместить платформы, чтобы создать иллюзию прокрутки и воссоздать платформы, которые находятся вне экранного обзора...
        else {
        ...
        //прыжок при попадании на другую платформу
        collides();

        if (player.isDead === true) gameOver();
      }

      //алгоритм работы пружины
    ...
    // горизонтальное движение платформ
    ...
    // функция обновления всех функций
  ...
  function showGoMenu() {
    var menu = document.querySelector('#gameOverMenu');
    menu.style.zIndex = 100000000;
    menu.style.visibility = 'visible';
    menu.style.color = "#f00";

    var scoreText = document.getElementById('go_score');
    scoreText.innerHTML = `Ваш счёт ${score} points!`;
  }
    
  
  // скрыть игру под меню
  function hideGoMenu() {
    var menu = document.querySelector('#gameOverMenu');
    menu.style.zIndex = -1;
    menu.style.visibility = 'hidden';
  }
  // показать поле с очками
  function showScore() {
    var menu = document.querySelector('#scoreBoard');
    menu.style.zIndex = 1;
  }

  // спрятать поле с очками
  function hideScore() {
    var menu = document.querySelector('#scoreBoard');
    menu.style.zIndex = -1;
  }
